...
``APIServerAdapter._maybe_expand_skill_command`` directly — it carries no
...
def _adapter() -> APIServerAdapter:
    # _maybe_expand_skill_command touches no instance attributes; skip __init__.
...
class TestNoExpansion:
    def test_non_string_content_passes_through(self):
        ...
        assert _adapter()._maybe_expand_skill_command(content, SESSION_ID) is None

    def test_message_without_leading_slash_passes_through(self):
        assert _adapter()._maybe_expand_skill_command("hello there", SESSION_ID) is None

    def test_lone_slash_passes_through(self):
        assert _adapter()._maybe_expand_skill_command("/", SESSION_ID) is None
        assert _adapter()._maybe_expand_skill_command("/   ", SESSION_ID) is None
...
class TestSkillExpansion:
    def test_known_skill_is_expanded(self):
        ...
        with patches[0], patches[1], patches[2], patches[3] as build:
            out = _adapter()._maybe_expand_skill_command("/site-audit run on example.com", SESSION_ID)
    ...
    def test_skill_without_instruction_uses_empty_string(self):
        ...
        with patches[0], patches[1], patches[2], patches[3] as build:
            out = _adapter()._maybe_expand_skill_command("/site-audit", SESSION_ID)
        assert out == "X"
    ...
    def test_leading_whitespace_before_slash_is_tolerated(self):
        patches = _patch_skills(skill_key="/site-audit", skill_msg="X", bundle_key=None)
        with patches[0], patches[1], patches[2], patches[3] as build:
            out = _adapter()._maybe_expand_skill_command("   /site-audit go", SESSION_ID)
        assert out == "X"
    ...
    def test_multiline_instruction_is_preserved(self):
        patches = _patch_skills(skill_key="/site-audit", skill_msg="X", bundle_key=None)
        with patches[0], patches[1], patches[2], patches[3] as build:
            _adapter()._maybe_expand_skill_command("/site-audit\nline one\nline two", SESSION_ID)
    ...
    def test_resolver_receives_command_resolution(self):
        ...
        with patches[0], patches[1], patches[2] as resolve, patches[3]:
            _adapter()._maybe_expand_skill_command("/site_audit now", SESSION_ID)
...
class TestBundlePrecedence:
    def test_bundle_wins_over_skill(self):
        ...
        with patches[0], patches[1] as build_bundle, patches[2], patches[3] as build_skill:
            out = _adapter()._maybe_expand_skill_command("/research deep dive", SESSION_ID)
    ...
    def test_bundle_build_failure_falls_back_to_skill(self):
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            out = _adapter()._maybe_expand_skill_command("/research", SESSION_ID)

...
def test_task_aware_omnio_cdp_fails_closed(monkeypatch):
    ...
    error = bu._resolve_backend_cdp({}, "task-17", session_name="same", bu_name="bu-hash")
...
def test_install_cli_forces_exact_package_pin(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bu, "_find_cli", lambda: [str(tmp_path / "browser-use")] if calls else None)
    monkeypatch.setattr(bu.shutil, "which", lambda name: "/managed/uv" if name == "uv" else None)
...
def test_timeout_stops_exact_named_harness(monkeypatch):
    ...
    monkeypatch.setattr(bu.subprocess, "run", fake_run)
    result = json.loads(bu.browser_exec("print('work')", session="shared", task_id="conv-1", timeout_s=5))
...
def test_omnio_cdp_uses_isolated_default_harness_instance(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hermes_constants.get_hermes_home", lambda: str(tmp_path / "home"))
    monkeypatch.setattr(bu, "_find_cli", lambda: ["/managed/browser-use"])
    monkeypatch.setattr(
        bu,
...
def test_omnio_stale_owner_reloads_exact_harness(monkeypatch, tmp_path):
    ...
    assert bu._configure_omnio_harness_dirs(
